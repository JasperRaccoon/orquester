...
- **A turn the CLI starts by itself streams before the turn exists.** When a background task or
  ...
  - `events.ndjson` keeps the old copies, so for Claude threads only, the client's
    `splitThreadItems` drops a finished assistant message whose text repeats an earlier finished
    one of the same turn and author. It is opted in from the thread head's adapter in `store.ts`;
    Codex narration may legitimately repeat itself.
