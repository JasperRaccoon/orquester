...
- **Seed credentials by conversion** (spike F4/F5) — the only credential path; there is no device-auth login flow. Field mappings are fixed in spec §4.
...
Continues in **Part 2** (frontend: Settings "Model proxy" UI, provider chips, seed-from-account picker, model chips + **per-launcher account chips** (claudex→Codex family, claudemix→Claude family; the `NewTabMenu` launcher-id→provider-family remap of spec §5), disabled rendering, `ApiClient`/store wiring, usage scanner/watcher extension) and **Part 3** (claudemix workflow + §8 routing + deploy). This Part 1 lands the daemon half of per-launch account routing — the prefix on seeds and the contributor mapping — so Part 2's chips are pure UI over an already-working mechanism. **There is no device-auth login flow anywhere in the build** — seeding from managed accounts is the only credential path (the Phase-1 `login/*` 501 stubs were removed).
...
- [ ] **Step 4:** Record results in the log doc; commit the log.

---

## Self-Review (performed at write time)

- **Spec coverage (Phase-2 scope):** §1 stock-binary install + rollback → Task 1; §4 seed-by-conversion + owner rule + **per-account prefix** → Tasks 2/3/5/7; §2 per-launch **account routing** (prefix stamping + contributor mapping — daemon half) → Tasks 2/5/7; §1 enable orchestration + seedHome wiring (Phase-1 carryover) → Task 4; §3 seed/openrouter routes + socket refusal → Task 7; §1 persistence-lost re-parent (Phase-1 review note) → Task 6; provider status/§3 → Task 5; live prefix-routing proof → Task 9. Continued in Parts 2–3 (both planned now, not deferred): the account/model **chip UI** (daemon half done here), wire-client, launcher rendering, usage attribution, claudemix workflow + §8 routing, deploy. No device-auth login and no cost-measurement task exist in the build (both removed by explicit decision).
