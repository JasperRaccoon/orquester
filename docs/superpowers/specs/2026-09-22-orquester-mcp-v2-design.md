...
v2 replaces that surface with **29 chat-native tools** (*Built: 31 — `search_sessions` and
`read_tool_output` were added after v2 shipped, §7.2*) that mirror the chat GUI one-to-one: what a
...
transcripts and subagents; wait for something to happen; read quota per account — an MCP client
can do with the same semantics, the same gates and the same error codes, because the tools are a
thin **in-process client of the daemon's own REST API** (§4.2). No screen text, no keystrokes, no
...
- Errors are `isError: true` with text `<CODE>: <message>` and `structuredContent {code, message,
  ...
  *Built: the todo store adds `NOT_FOUND` (a list id that does not exist; the message names the id:
  ...
  through `result.ts`'s `TodoError` mapping, which `TodoTools` lets through; `read_tool_output`
  answers `NOT_FOUND` too, for an item the host does not have (§7.2). A daemon answer that
...
- **Size.** Every result's text is capped at 60 000 UTF-8 bytes (Claude Code discards MCP results
  ...
  *Built: `get_turn_diff` takes no `maxChars` — its diff is bounded by the result budget itself,
  ...
  `read_tool_output` and every session detail bound themselves too (§6.2, §6.3, §7.1, §7.2, §7.8,
  §7.9). A result still over the cap keeps its leading bytes, never cutting a character, and ends
  with `… [truncated: N bytes over the 60 000-byte cap]`; its `structuredContent` is then only
...
| `read_tool_output` — *added after v2 shipped (2026-09-23)* | `sessionId`, `itemId` (a tool row's `outputItemId`, §7.6), `offset? = 0`, `maxBytes? = 40000` (1–55 000) | `{itemId, kind: "command-output" \| "message" \| "payload", text, offset, totalBytes, nextOffset?, running?, truncated?}` — the timeline's "Load full output" (`ActivityRows.tsx`). `GET /api/sessions/:id/items/:itemId` — the unslimmed item (the GUI spec's §5.6) — after the session is checked to be a chat tab. The text, in this order: (1) `kind: "command-output"` for a `command_execution` activity whose own data carries output, unless the item is stored slimmed (`payload.truncated === true`: an update, which ingestion persists already cut, so its data is the preview): that output whole — the first place, in the preview's reading order, that holds output in the unslimmed item, from one list in `packages/api/src/agent-chat/command-output.ts` which `commandDisplayDetail`'s preview and `commandOutputText` both read (usually, not always, the text the preview was cut from: the slimmer's allow-list rebuild can leave another place first, as Grok's does) — and untrimmed: `stdout` then `stderr`, and ACP content blocks, each start a line of their own. (2) *Built (plan `2026-09-23-mcp-output-drill-in-and-housekeeping`, item 7):* else, for a command's row naming its call (`payload.toolUseId`) — a `command_execution` activity, or a `tool.output` chunk whose `streamKind` is `command_output` — the call's streamed output as `command-output`, when it streamed any — its `tool.output` chunks (`payload.delta`), the only copy of a Claude background shell's output (tailed from the CLI's file: ≤ 1 MiB, then one notice) and of a running command's output so far, joined by the host from the log (`GET /api/sessions/:id/items/:itemId/output` → `ThreadItemOutputResponse {toolUseId, output, complete, truncated}`), with `running: true` while no `tool.completed` exists for the call and `truncated: true` when the host's 8 MiB cap (`THREAD_ITEM_OUTPUT_MAX_BYTES`) cut the join. Never for a file change (fix round 1): Claude streams every Edit/Write result's text as `file_change_output` ("File created successfully at: …", fixture `claude/14a`), which the host joins too — the join is by call, not by stream — but which is no command's output; such an item answers its payload, the edit's input included, as the GUI's viewer shows it. A 404 there — the host's own `ITEM_NOT_FOUND`, or, until its drain-restart, a host that predates the route answering its generic route miss (`THREAD_NOT_FOUND` "No route for GET …") — skips this step and is never an error; any other failure goes through `daemonError`. (3) Any other item answers the GUI viewer's text (`fullOutputText`, `AgentChatView.tsx`): a message's text (`message`); a string payload as it is, else `JSON.stringify(payload, null, 2)`, else the row's summary (`payload`) — for a command too when neither step found output. `offset`, `nextOffset` and `totalBytes` are UTF-8 byte offsets into that text (a lone surrogate reads as U+FFFD). A window never splits a character: it holds at most `maxBytes` bytes — one whole character (up to 4 bytes) when `maxBytes` is narrower than it, so paging always advances — and shrinks, to the last character that fits, until the result fits the 60 000-byte cap (escaped as JSON, most control characters take 6 bytes); an `offset` inside a character starts at that character, and the result's `offset` says where. `nextOffset` is present only when more remains; an `offset` past `totalBytes` is `INVALID_ARGUMENT` naming it (at `totalBytes` the text is empty). A 404 from the item route — the host's `THREAD_NOT_FOUND` "No item '<id>'." — is `NOT_FOUND`, saying the item is gone or never existed and that item ids come from `read_transcript`'s `outputItemId` (the id quoted cut to 100 code points); any other failure goes through `daemonError`. Every page re-reads the item, and a streamed output's join: the host reads both back from the thread's log. Annotated as a read (`openWorldHint: false`). |
...
{ turn: number | null, turnId, kind, createdAt, agentId?,
  kind = "user"      → text, attachments?: [{name, type}]
       ...
       | "tool"      → tool: { type /* itemType */, title, status, command?, detail?, changedFiles? }   // one entry per toolUseId, latest state
                       /* Built: outputItemId? — where the whole of what the read cut, or the call streamed, is read: read_tool_output */
...
`olderTurns` is always returned: the started turns before the first turn the result delivers — `start − 1`, or
`coveredTurns[0] − 1` when the shed dropped rows — so `beforeTurn: olderTurns + 1` never skips a turn and always moves
back (the final review found that `start − 1` skipped the range's first turns whenever the shed had dropped them all). A
row with no turn belongs to the range by its time: at or after turn `start`'s `requestedAt` (from the very start when
`start` is 1) and, when `end < turnCount`, before turn `end + 1`'s. When `history.indexed` and `hasOlder` are true and
`start ≤ oldestRetainedOrdinal` (the window's oldest turn may be partial), `history.ts` pages `GET …/history` through
`DaemonApi`: the first page ends below turn `end + 1` — a cursor minted from its turn record,
`{threadId, beforeAnchorAt: requestedAt, beforeTurnId}` — when `end < oldestRetainedOrdinal`, else at the window's
boundary (no cursor); each next page follows `page.beforeCursor` until turn `start` is whole — the cursor's turn
`k ≤ start` without `beforeSeq`, `k < start` with it — or the cursor is null, at most `HISTORY_PAGES_PER_READ` = 5 pages
a call. Every page asks for `turns = min((cursorMinted ? end + 1 : oldestRetainedOrdinal + 1) − start + 1, 100)`: a soft
cap one turn below `start`, because the host puts `beforeSeq` in every cursor it mints (`blockCursor`), so a page capped
at turn `start`'s own first line named turn `start` with it and the stop rule read a second page; capped one turn lower,
a range the 400-activity block holds is read in one page, and a range from turn 1 has no cap at all and ends on a null
cursor. (The design's `end − start + 1` cost two or three pages where one sufficed — found by the final review. Its
`end + 1 < oldestRetainedOrdinal` ended the first page of a range just before the window's oldest turn at the window's
boundary: the walk then waded through that turn's evicted rows first, and a large one stopped it before turn `end`,
which the page-limit sentence below then named as the large turn — found while fixing that sentence.) Pages merge
under the window by item id, the window's copy winning (the newer state of the same
...
back whole, never "the thread's first turn reached" — except as the first page of the cursor minted from turn `end + 1`
(final review): the host plans a block from the activities below its end, which its own cursors and the window's
boundary always have and turn `end + 1` may not — a resumed conversation's replayed first turns can be pure chat — so
that one empty page is the thread's start reached, and the parent view's net decides); `indexed:false` with turns of the
range that have no row in the window — for a drill-in, the subagent's own span, as below; an index that has not caught
up with the thread — `indexed` true, `hasOlder` not true and `totalTurns` below the thread's started turns, as every
thread is after an index rebuild until its catch-up reaches it — which cannot place the window's rows, so the range's
turns up to the window's oldest (`oldestRetainedOrdinal`, else the turn of the window's oldest activity row that
retention could drop — a compaction marker of either spelling, `isCompactionActivity`, is one it keeps — or, for a
drill-in, of the subagent's own oldest row left, anchors aside: an agent's rows are kept in windows of their own, its
last 200 and the newest 2 000 across agents, which drop the oldest first and can begin turns after the parent's; the
snapshot cannot tell a full window by counting, because the host serves it with the replaced `tool.updated` rows already
...
such net: a subagent has no row in the turns it did not run in); or the page limit, whose sentence names the
`beforeTurn`/`turns` that reads the turns left, or — when the walk never got out of turn `end` — says that turn is
larger than one call reads, returns its latest rows, and names the call for the turns before it, so following the hints
never repeats a call. (Follow-up, plan `2026-09-23-mcp-output-drill-in-and-housekeeping`, items 2–3: a drill-in with no
index was judged by `missingTurns`, which reads any row, so the parent's rows passed for the subagent's and a partial
span was served with no flag; the empty page was taken for the thread's first turn, and only the net — which a drill-in
no longer runs — named its turns; and a Claude subagent that had ended with no row left was bounded by the oldest row
any agent kept rather than by its end. The first build bounded an agent by any end; the review found rows following a
stamped end — the adapter gap above, reported to the owner — so only Claude's end bounds. A task row's stamp names its
OWNER — Claude stamps a task launched inside a subagent with that subagent's id — which is why `taskId` decides first.)
A failed page is never a tool error, the rows read are served. A host without `history` reads as before, plus
`olderTurns`. Both new fields are in the byte frame — a shed result's measured with the widest `olderTurns`
(`turnCount`), as its value is not known before the shed — and the sentence's bytes are held back — on a whole result as
its own `hint`, on a shed one before the shed hint — so the answer still keeps within `maxChars`.*
...
slimmer's promise that `GET …/items/:itemId` holds more (the GUI spec's §5.6) — among the rows the host stores whole: its
...
`tool.output` chunks, which are never entries and which only the host can join whole (§7.2 `read_tool_output`, step 2).
The transcript records the `toolUseId` of every `tool.output` row of `command_output` in the view's scope — wherever it
falls, not only in the range: a background shell outlives the turn that launched it, and its chunks carry whichever turn
is live when they arrive; a file change's `file_change_output` never counts (fix round 1: it is no command's output, and
`read_tool_output` never answers it as one) — and an entry of such a call that has no id from the rule above gets
`outputItemId` = its latest lifecycle row in the range that says it is a command (`itemType: "command_execution"`, what
`read_tool_output` gates its step 2 on): a running call's latest update or its start, or a completion nothing cut; else
its latest chunk in the range. A start or an update is safe here: `read_tool_output` never reads a stored-slimmed row's
...
`wait_for_session`) · requests 3 · usage 2 · files 2 · todos 5. *Built: 31 — search 1
(`search_sessions`, the command palette's `?` search, added after v2 shipped, §7.2), listed in
`tools/list` between `compact_session` and `send_message`; and `read_tool_output` (the timeline's
"Load full output", added after v2 shipped, §7.2), listed right after `read_transcript`.* Every
composer chip, banner button, header action and `+`-menu action in the chat GUI maps to exactly one
of them; the only GUI-reachable chat action without a tool is Ctrl+B "run this command in the
background" (`/background`, Claude only) — omitted as YAGNI, trivially addable.
