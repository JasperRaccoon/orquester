...
The transcript records the `toolUseId` of every `tool.output` row in the view's scope — wherever it falls, not only in
the range: a background shell outlives the turn that launched it, and its chunks carry whichever turn is live when they
arrive — and an entry of such a call that has no id from the rule above gets `outputItemId` = its latest lifecycle row in
the range: a running call's latest update or its start, or a completion nothing cut. A start or an update is safe here:
`read_tool_output` never reads a stored-slimmed row's own data as the output, and resolves the call from any of its
rows. A background shell's rows carry its task id as `agentId`, so they are in its drill-in only, as in the GUI; its
completion is always cut (the allow-list drops `input` and `background`) and holds no output, so the id is the
completion's and the answer is the join.*
