...
folding every word behind "Worked for". In a Claude thread, a finished assistant message whose text
repeats an earlier finished one of the same turn and author is dropped as a re-emission: hosts
before the Claude pre-turn-stream fix (§4.5, Claude *Steering*) wrote the opening paragraph of a
CLI-started turn a second time at `result`, and `events.ndjson` keeps it (`splitThreadItems`). No
other provider ever re-emitted a message, and Codex narration may legitimately repeat itself.*
