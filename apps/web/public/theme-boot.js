 ...
 *
 * A separate file rather than an inline <script> on purpose: the production
 * Caddy CSP is `script-src 'self'` (deploy/Caddyfile), which blocks inline
 * scripts, and /etc/caddy/Caddyfile is reconciled by hand on deploy — a
 * hash-pinned inline script would silently stop running.
 */
(function () {
  try {
