...
| `orchestration/attachment-lines.ts` | **W1** | §4.1 attachment delivery, pure: the one `Attached file: <name> (<absolute path>)` line (name flattened to a single line, capped at 255), `appendAttachmentLines` — AFTER the text, never before, so a `/command` still dispatches (§4.6.9) — `partitionAttachments` on the adapter's required `ingestsAttachment`, and `ATTACHMENT_LINES_MAX_CHARS` for an adapter that bounds its whole input. `sendTurnEffect` resolves and stats every attachment on every sending path, then renders with it; the native answer fold uses the same line. An adapter never prints a path of its own. |
