...
| `schema.ts` | The tables (`INDEX_SCHEMA_VERSION` — bump it for ANY statement change): `threads` (the per-thread cursor `last_seq`/`last_byte`, plus `open_turn_id`, `revert_seq` and `inflight` — the requested-but-unstarted turns and unclaimed prompts no `turns` row can hold, JSON), `turns` (ordinal + `[first_byte, end_byte)`), `items` (each activity's latest line), `message_docs` (each message's span and the keyed side of `messages_fts`), `markers` (compaction phases), and the two FTS5 tables (`unicode61 remove_diacritics 2`). |
...
- **`state.json` and `index.sqlite` are caches of `events.ndjson`, never authorities** (invariant
  ...
  for the same log, and `INDEX_SCHEMA_VERSION` for any change to the index's statements. The index
