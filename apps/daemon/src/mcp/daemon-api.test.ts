...
test("uploadAttachment delegates to the agent chat service", async () => {
  const calls: unknown[] = [];
  const agentChat = {
...
test("a thrown upload is HOST_UNAVAILABLE, as a thrown daemon call is: the cause is logged, never returned, and the stream is released", async (t) => {
  ...
  const gone = chatApi({ uploadAttachment: async () => { throw new Error("connect ENOENT /var/lib/orquester/daemon/agent-host.sock"); } });
  ...
  const drains = chatApi({ uploadAttachment: async (_id: string, _q: unknown, body: Readable) => { for await (const chunk of body) void chunk; return { status: 200, value: null }; } });
  ...
  const capped = chatApi({ uploadAttachment: async () => { throw new UploadTooLargeError(); } });
...
test("a cap refusal wrapped as another error's cause is still 413 UPLOAD_TOO_LARGE, with the refusal's own message", async (t) => {
  ...
  for (const wrapped of [new HostUnavailableError("socket hang up", new UploadTooLargeError()), new Error("the upload stream failed", { cause: new UploadTooLargeError() })]) {
    ...
    const up = await chatApi({ uploadAttachment: async () => { throw wrapped; } }).uploadAttachment("s1", { name: "big.bin" }, body);
  ...
  const eio = await chatApi({ uploadAttachment: async () => { throw new HostUnavailableError("EIO", new Error("EIO: i/o error, read")); } }).uploadAttachment("s1", { name: "b.log" }, Readable.from([]));
...
test("a path attachment the seam refuses, or that fails mid-read, leaves no file open", { skip: !existsSync("/proc/self/fd") && "needs /proc/self/fd" }, async (t) => {
  ...
  const hangsUp = { uploadAttachment: async (_id: string, _q: unknown, body: Readable) => { for await (const chunk of body) { void chunk; throw new Error("socket hang up"); } return { status: 200, value: null }; } };
