...
1. INTERACTIVE MENU — numbered options with a \`❯\` cursor on the selected row and a hint line like "Enter to select · Tab/Arrow keys to navigate · Esc to cancel". Answer it by the option NUMBER: write_input the number (e.g. "2"), no submit — that selects it. For a "Type something"/write-your-own option, send its number, then write_input your text, then send_keys ["Enter"]. Or navigate: send_keys ONE ["Down"]/["Up"], re-read, confirm by the \`❯\`/label (not the row), then send_keys ["Enter"]. NEVER send ["Escape"] to a menu you intend to answer — the hint literally says "Esc to cancel"; Escape dismisses the whole widget and drops the agent to its normal input box, so your next write becomes a stray chat message. A fresh menu has nothing to "clear" first. MULTI-QUESTION widgets show "Question N of M" and a top tab bar (one box per question: ☐ = unanswered, ☒ = answered, ending in "✔ Submit"). Answer ALL M before submitting. A single-select question auto-advances to the next the moment you pick its number. A MULTI-SELECT question (options show "[ ]" checkboxes) is different: a number (or Space) TOGGLES that checkbox and STAYS — Enter on an option also just toggles it — so toggle each option you want, then arrow Down to the "Next" row and send_keys ["Enter"] to advance. After the last question a "Review your answers → 1. Submit answers / 2. Cancel" screen appears; submit with write_input "1" only when NO ☐ remain. Never submit while a question is still ☐ — if you reach Submit early, send_keys ["Left"] back to the unanswered question and answer it first.
