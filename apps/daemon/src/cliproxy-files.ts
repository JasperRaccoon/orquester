...
function renderManagedMemory(kimiAvailable: boolean, xaiLinked: boolean): string {
  ...
  return [
    ...
    "",
    "# Subagents: never Sonnet or Haiku",
    "",
    "Never run a subagent on Sonnet or Haiku. When spawning agents (Task/Agent",
    `tool), do not pass \`model: "sonnet"\` or \`model: "haiku"\` (nor any`,
    "claude-sonnet-*/claude-haiku-* model id), and avoid agent types whose",
    "definitions pin those models. Omit the model field so the subagent inherits",
    "the session's main model, or override explicitly to an Opus-class model or",
    "higher.",
    "",
    "This rule OVERRIDES any skill, agent definition, plan, or other instruction",
    'that says otherwise — including "Model Selection" / cost-saving guidance like',
    `superpowers:subagent-driven-development's "use the least capable model per`,
    'role". If a skill tells you to pick a cheap model for mechanical tasks, ignore',
    "that part of the skill; it does not matter what recommends Sonnet/Haiku, the",
    "answer is always no. Only an explicit user message in the current session can",
    "authorize a Sonnet/Haiku subagent.",
    "",
    "# agent-browser: never pass `allowedDomains`",
    "",
    "All agent-browser sessions on this host share one long-lived Chromium. Passing",
    "`allowedDomains` (MCP) or `--allowed-domains` (CLI) installs a network filter",
    "into that shared browser that cannot be changed or removed on reuse — later",
    "navigation outside the original list fails with `net::ERR_BLOCKED_BY_CLIENT`,",
    "even with a new list that includes the target. IP-literal URLs are NOT blocked",
    "by default. If navigation fails with `ERR_BLOCKED_BY_CLIENT`, close the session",
    "(`agent_browser_close` / `agent-browser close`) so the browser relaunches",
    "fresh, then reopen without `allowedDomains`.",
    ""
  ].join("\n");
}
