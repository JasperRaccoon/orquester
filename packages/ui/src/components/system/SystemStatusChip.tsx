...
/**
 * Top-bar host chip: CPU% and memory% of the machine the daemon runs on, with
 * the full resource breakdown in a popover.
 *
 * Hidden entirely below `sm` — the phone header already carries menu, project,
 * search, agents, usage and settings, and Settings → System is the phone-side
 * surface for this. Hidden too when the host can't report (`supported: false`,
 * i.e. anything but Linux) or when nothing has been read yet, following the
 * UsageWidget/AttentionCenter convention of staying out of the top bar rather
...
export const SystemStatusChip: React.FC = () => {
  ...
  const trigger = (
    <span
      title={label}
      aria-label={label}
      className="flex h-7 items-center gap-2 rounded-md px-2 text-xs font-medium text-neutral-300 hover:bg-neutral-800"
    >
  ...
  return (
    <AdaptiveMenu title="Host" trigger={trigger} align="right" width="w-72">
